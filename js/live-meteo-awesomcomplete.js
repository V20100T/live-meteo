...
var url_list_cities = 'http://www.prevision-meteo.ch/services/json/list-cities';
var stream_ratp_time = 1000 * 35;



var input = document.getElementById("cities_input");
var ajax = new XMLHttpRequest();
ajax.open("GET", url_list_cities, true);
ajax.onload = function() {
  console.log('>> ajax onload ' + url_list_cities);
  var list = formatApiListCities(JSON.parse(ajax.responseText));
  var lineAwesomplete = new Awesomplete(
    document.getElementById("cities_input"), {
      list: list,
      minChars: 0,
      maxItems: 50,
      autoFirst: true,
...
function formatApiListCities(rep) {
  list = [];
  console.log('>> formatApiListCities : ', rep);
  if (rep == 'undefined') {
    return list;
  }

  _.each(rep, function(itemList, type) {
  ...
  console.log('<< formatApiListCities : ', list);
  return list;
};
